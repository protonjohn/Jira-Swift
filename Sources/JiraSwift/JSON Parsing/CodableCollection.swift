...
public indirect enum CodableCollection: Equatable {
    case int(Int)
    case bool(Bool)
    case data(Data)
    case double(Double)
...
extension CodableCollection: ExpressibleByArrayLiteral {
    public init(arrayLiteral elements: CodableCollection...) {
        self = .list(elements)
    }
}

extension CodableCollection: ExpressibleByDictionaryLiteral {
    public init(dictionaryLiteral elements: (String, CodableCollection)...) {
